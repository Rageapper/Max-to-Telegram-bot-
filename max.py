...
class MaxClient:
    ...
    def connect(self, _f=None):
        ...
        # Проверяем ответ на user_agent
        try:
            ua_resp = json.loads(user_agent_response)
            if ua_resp.get('payload', {}).get('error'):
                print(f"[DEBUG] Ошибка при отправке user_agent: {json.dumps(ua_resp, ensure_ascii=False, indent=2)}")
        except:
            pass
        ...
        connect_response = json.loads(self.websocket.recv())
        
        # Проверяем наличие ошибок в ответе
        if connect_response.get('payload', {}).get('error'):
            error_code = connect_response['payload'].get('error', '')
            error_msg = connect_response['payload'].get('localizedMessage', 'Ошибка подключения')
            raise ValueError(f"Ошибка подключения: {error_code} - {error_msg}")
        
        p = connect_response['payload']
        
        if 'profile' not in p:
            raise ValueError(f"Неожиданный формат ответа сервера. Отсутствует 'profile' в payload: {json.dumps(connect_response, ensure_ascii=False, indent=2)}")
    ...
    def run(self):
        ...
        try:
            self.connect()
            print("[ПОДКЛЮЧЕНИЕ] WebSocket соединение установлено")
        except Exception as e:
        ...
        print("[ЗАПУСК] Слушатель сообщений запущен")
    ...
    def _start_auth(self, phone_number) -> dict:
        ...
        Usage:
            ...
            ```
        """
        # Отключаемся перед новым подключением, если уже подключены
        if self._connected:
            self.disconnect()
        ...
        response_raw = self.websocket.recv()
        response = json.loads(response_raw)
        
        # Отладочный вывод для диагностики
        if response.get('payload', {}).get('error'):
            print(f"[DEBUG] Ответ сервера: {json.dumps(response, ensure_ascii=False, indent=2)}")
    ...
    # region auth()
    def auth(self, phone_number: str, max_retries: int = 5):
        """
        ...
        # Повторные попытки при ошибке service.unavailable
        code_resp = None
        for attempt in range(max_retries):
            try:
                ...
                if code_resp.get('payload', {}).get('error'):
                    error_code = code_resp['payload'].get('error', '')
                    error_msg = code_resp['payload'].get('localizedMessage', '')
                    
                    # Если ошибка service.unavailable - пробуем снова
                    if error_code == "service.unavailable" and attempt < max_retries - 1:
                        print(f"Сервис временно недоступен. Повторная попытка... (попытка {attempt + 1}/{max_retries})")
                        # Переподключаемся
                        self.disconnect()
                        continue
                    else:
                ...
                if attempt < max_retries - 1:
                    print(f"Ошибка при подключении: {e}. Повторная попытка... (попытка {attempt + 1}/{max_retries})")
                    self.disconnect()
                else:
                    raise
